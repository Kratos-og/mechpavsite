...
const LeftMenu = props => {
    ...
    return (
        <div className={`fixed w-screen h-screen top-0 z-50 ${props.show ? 'left-0' : 'left-full'}`}>
            ...
            <AnimatePresence>
                {props.show &&
                    <motion.div style={{ left: 0 }} exit={{ left: '-30%', transition: { duration: 0.3 } }} className="w-[20rem] fixed top-0 bg-black z-50 h-screen p-8 border-r">
                        <div className="w-full relative">
                            <motion.div
                                initial={{ left: '10%' }}
                                animate={{ rotateZ: '180deg', left: '75%', transition: { duration: 0.4 } }}
                                exit={{ left: '10%', rotateZ: 0, opacity: 0, transition: { duration: 0.2 } }}
                                className="p-4 top-4 absolute bg-white rounded-full flex items-center justify-center cursor-pointer" onClick={() => props.toggleMenu(true)}>
