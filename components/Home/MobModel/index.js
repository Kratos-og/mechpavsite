...
const MobModel = props => {
    ...
    const onScroll = (y) => {
        ...
        if (val < 33) {
            if (prevScroll < val) {
                //down
                let perc = (val / 33) * 100;
                //goal -> camOrbit = 80deg 110deg 2m, camTarget = 0m 1.1m -0.5m
                theta = 20 - (perc * (60 / 100));
                phi = 90 + (perc * (5 / 100));
                radius = 6 - (perc * (4 / 100));
                camPhi = 0.7 + (perc * (0.1 / 100))
                ...
                theta = -60 - (perc * (60 / 100)); //-60
                phi = 95 + (perc * (5 / 100));  //110
                radius = -4 - (perc * (4 / 100)) //2
                camPhi = 0.8 + (perc * (0.1 / 100)); //1.1
                camRad = 0.3 - (perc * (0.2 / 100)) // -0.5
                orbitModel(theta, phi, radius, 0, camPhi, camRad);
            }
        }
            ...
            if (prevScroll < val) {
                ...
                camPhi = 0 - (perc * (0.8 / 100));
                camRad = 0.6 + (perc * (0.6 / 100))
                ...
                camPhi = 0.4 - (perc * (0.6 / 100));
                camRad = 0.8 + (perc * (0.6 / 100))
                orbitModel(theta, phi, radius, camTheta, camPhi, camRad);
            }
        }
            ...
            if (prevScroll < val) {
                ...
                theta = -215 + (perc * (130 / 100));
                phi = 40 + (perc * (45 / 100));
                radius = 7 - (perc * (1 / 100));
                camTheta = 0.6 + (perc * (1 / 100));
                camPhi = 0.3 + (perc * (0.7 / 100));
                camRad = 0.7 - (perc * (0.9 / 100))
                orbitModel(theta, phi, radius, camTheta, camPhi, camRad);
            }
            else if (prevScroll > val) {
                //up
                ...
                radius = 6 - (perc * (1 / 100));
                camTheta = 0 + (perc * (1 / 100));
                camPhi = 0.5 + (perc * (1 / 100));
                camRad = 0.2 - (perc * (1.1 / 100))
                orbitModel(theta, phi, radius, camTheta, camPhi, camRad);
            }
        }
        prevScroll = val;
    ...
    return (
        <div className='w-full h-full fixed top-0 pointer-events-none overflow-hidden'>
            ...
            <model-viewer loading="eager" style={{ opacity: show ? 1 : 0 }} id="mech-model" src="/assets/models/invaderPav.glb" camera-orbit="0deg 100deg 0m" camera-target="0m 0.6m 0.4m">
            </model-viewer> 
        </div>
    )
}
