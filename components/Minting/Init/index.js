...
const Init = props => {
    ...
    return (
        <div className="flex h-[90%] items-center justify-center px-10 gap-10 relative max-md:flex-col">
            ...
            <div className="w-[25%] max-md:w-full h-[85%] mt-5 border-2 border-[#423F3E] text-[0.65rem] font-black tracking-widest uppercase bg-black">
                ...
                <div className="px-2 py-2 h-[90%]">
                    <div className="px-3 py-2 overflow-auto custom-scroll h-full">
                        ...
                        {activeIndex >= 14 && confirmedQty > 0 && !confirm && <Confirm confirm={setConfirm} onConfirm={setConfirmedQty} />}
                        {confirm === "yes" && <>
                            <p className="text-gray-500 pb-1 pt-5">
                                {activeIndex >= 16 && <Text index={16} onDone={setActiveIndex} text={"> < INFO > "} speed={30} />}
                            </p>
                            <p className=" text-[#14fecdff]">
                                {activeIndex >= 17 && <Text index={17} onDone={setActiveIndex} text={`> confirmed . . . ! !`} speed={30} />}
                            </p>
                            {activeIndex >= 16 && !done && <Mint done={setDone} />}
                            {done && <>
                                <p className="text-gray-500 pb-1 pt-5">
                                    {activeIndex >= 18 && <Text index={18} onDone={setActiveIndex} text={"> < completed minting > "} speed={30} />}
                                </p>
                                <p className=" text-[#14fecdff]">
                                    {activeIndex >= 19 && <Text index={19} onDone={setActiveIndex} text={`> successfully minted YAYAAAA . . . ! ! `} speed={30} />}
                                </p>
                            </>}
                        </>}
                        <div>
